...
function isConflict(leave, sessions) {
	const d1 = new Date(leave.from),
		d2 = new Date(leave.to)
	const buffer = 3 * 24 * 60 * 60 * 1000
	return sessions.some((s) => {
		const sd = new Date(s.date)
		return (
			sd >= new Date(d1.getTime() - buffer) &&
			sd <= new Date(d2.getTime() + buffer)
		)
	})
}
...
export default function Leaves() {
	...
	const handleAddLeave = () => {
		...
		toast(
			conflict
				? "⚠ Leave added — CONFLICT DETECTED!"
				: "✓ Leave request submitted",
			conflict ? "warn" : "success",
		)
		setModal(false)
		setForm(EMPTY_FORM)
	}
	...
	return (
		<div>
			<div className="flex items-start justify-between mb-7">
				<div>
					...
					<div className="text-xs text-[#7d8590] mt-1 font-mono">
						CIE-II window: Nov 17–22, 2025 (±3 day restriction)
					</div>
				</div>
				<button
			...
			<div className="bg-[#f85149]/10 border border-[#f85149]/30 px-4 py-3 mb-5 font-mono text-xs text-[#f85149]">
				⚠ POLICY: No leave approved within 3 days before or after exam
				window (Nov 14 – Nov 25, 2025)
			</div>
			...
			<div className="space-y-2 mb-6">
				{pending.length === 0 && (
					<div className="bg-[#161b22] border border-[#30363d] p-4 text-center text-[#7d8590] font-mono text-sm">
				...
				{pending.map((l) => {
					...
					return (
						<div
							key={l.id}
							className="bg-[#161b22] border border-[#30363d] p-4 flex flex-wrap gap-4 items-center"
						>
							...
							<div className="font-mono text-[11px] text-[#7d8590]">
								...
								→{" "}
								{new Date(l.to).toLocaleDateString("en-IN", {
									day: "2-digit",
									month: "short",
								})}
							</div>
							{conflict ? (
								<span className="font-mono text-[10px] px-2.5 py-1 bg-[#f85149]/15 text-[#f85149] border border-[#f85149]/30 uppercase">
									✗ Window Conflict
								</span>
							) : (
								<span className="font-mono text-[10px] px-2.5 py-1 bg-[#3fb950]/15 text-[#3fb950] border border-[#3fb950]/30 uppercase">
									✓ Safe Window
								</span>
							)}
							{isAdmin && (
								<div className="flex gap-2">
									<button
										...
										className="font-mono text-[10px] text-[#3fb950] border border-[#3fb950]/40 hover:bg-[#3fb950]/10 px-3 py-1 transition-colors"
									>
										Approve
									</button>
									<button
										...
										className="font-mono text-[10px] text-[#f85149] border border-[#f85149]/40 hover:bg-[#f85149]/10 px-3 py-1 transition-colors"
									>
										Reject
									</button>
								</div>
				...
				<table className="w-full border-collapse">
					<thead>
						<tr className="bg-[#1c2128] border-b border-[#30363d]">
							{["Faculty", "Leave Dates", "Reason", "Status"].map(
								(h) => (
									<th
										key={h}
										className="text-left px-4 py-2.5 font-mono text-[10px] uppercase tracking-[1.5px] text-[#7d8590] font-medium"
									>
										{h}
									</th>
								),
					...
					<tbody>
						{others.map((l) => (
							<tr
								key={l.id}
								className="hover:bg-[#1c2128] border-b border-[#30363d] last:border-0"
							>
								<td className="px-4 py-[10px] text-[13px] font-medium">
									{l.facultyName}
								</td>
								...
								<td className="px-4 py-[10px] text-xs text-[#7d8590]">
									{l.reason}
								</td>
								<td className="px-4 py-[10px]">
									<span
										className={`pill ${statusPill(l.status)}`}
									>
										{l.status}
									</span>
								</td>
						...
						{others.length === 0 && (
							<tr>
								<td
									colSpan={4}
									className="text-center py-6 text-[#7d8590] font-mono text-sm"
								>
				...
				>
					<div className="space-y-4">
						<div className="flex flex-col gap-1.5">
							<label className="font-mono text-[10px] uppercase tracking-[1px] text-[#7d8590]">
								Faculty Member
							</label>
							<select
